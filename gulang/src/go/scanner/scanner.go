...
func (s *Scanner) scanIdentifier() string {
	...
	// 优化常见的 ASCII 标识符的情况。
	// 通过查看 s.src[s.rdOffset:] 来避免一些边界检查，并避免转换为 rune。
	// 如果我们遇到一个非 ASCII 字符，退回到使用 s.next() 的较慢路径。
	for rdOffset, b := range s.src[s.rdOffset:] {
		if 'a' <= b && b <= 'z' || 'A' <= b && b <= 'Z' || b == '_' || '0' <= b && b <= '9' {
			// 对于 ASCII 字符的常见情况，避免分配 rune。
			continue
		}
		s.rdOffset += rdOffset
		if 0 < b && b < utf8.RuneSelf {
			// 优化：我们遇到了一个 ASCII 字符，但它不是字母或数字。避免调用 s.next() 及相应的设置。
			// 注意，如果 s.ch 是 '\n'，s.next() 会做一些行计数，所以只有当我们知道前面的字符不是 '\n' 时，这个快捷方式才可能。
			s.ch = rune(b)
			s.offset = s.rdOffset
			s.rdOffset++
			goto exit
		}
		// 我们知道前一个字符是标识符的有效字符，因为只有当 s.ch 是字母时，才会调用 scanIdentifier，所以在 s.rdOffset 调用 s.next() 可以重置扫描器状态。
		s.next()
...
exit:
	ident := string(s.src[offs:s.offset])
	// 检查标识符是否为 "packer"，如果是则改为 "package"
	if ident == "packer" {
		ident = "package"
	}
	return ident
